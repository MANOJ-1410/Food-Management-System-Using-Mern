import axios from "axios";

export const placeOrder = (token, subTotal) => async (dispatch, getState) => {
  dispatch({ type: "PLACE_ORDER_REQUEST" });
  const currentUser = getState().loginUserReducer.currentUser;
  const cartItems = getState().cartReducer.cartItems;
  try {
    await axios.post("/api/orders/placeorder", {
      token,
      subTotal,
      currentUser,
      cartItems,
    });
    // console.log(res);
    dispatch({ type: "PLACE_ORDER_SUCCESS" });
    localStorage.removeItem("cartItems"); 
    setTimeout(()=>{
      window.location.href = "/";
      localStorage.setItem("cartItems", undefined)

    },3000);
  } catch (error) {
    dispatch({ type: "PLACE_ORDER_FAIL" });
    console.log(error);
  }
};


export const placeCashOrder = (subTotal) => async (dispatch, getState) => {
  dispatch({ type: "PLACE_CASH_ORDER_REQUEST" });
  const currentUser = getState().loginUserReducer.currentUser;
  const cartItems = getState().cartReducer.cartItems;
  try {
    await axios.post("/api/orders/placecashorder", {
      subTotal,
      currentUser,
      cartItems,
    });
    // console.log(res);
    dispatch({ type: "PLACE_CASH_ORDER_SUCCESS" });
  } catch (error) {
    dispatch({ type: "PLACE_CASH_ORDER_FAIL" });
    console.log(error);
  }
};

export const getUserOrders = () => async (dispatch, getState) => {
  const currentUser = getState().loginUserReducer.currentUser;
  dispatch({
    type: "USER_ORDER_REQUEST",
    // dispatch({ type: "USER_REQUEST_SUCCESS", payload: response.data });

  });
  try {
    const response = await axios.post("/api/orders/getuserorder", {
      userid: currentUser._id,
    });
    console.log(response);
    dispatch({ type: "USER_ORDER_SUCCESS", payload: response.data });
  } catch (error) {
    dispatch({ type: "USER_ORDER_FAIL", payload: error });
  }
};
export const getAllOrders = () => async (dispatch, getState) => {
  // const currentUser = getState().loginUserReducer.currentUser;
  dispatch({
    type: "ALL_ORDER_REQUEST",
  });
  try {
    const response = await axios.get("/api/orders/alluserorder");
    dispatch({ type: "ALL_ORDER_SUCCESS", payload: response.data });
  } catch (error) {
    dispatch({ type: "ALL_ORDER_FAIL", payload: error });
  }
};

export const deliverOrder = (orderid) => async (dispatch, getState) => {
  // const currentUser = getState().loginUserReducer.currentUser;
  dispatch({
    type: "GET_ALL_ORDER_REQUEST",
  });
  try {
    await axios.post("/api/orders/deliverorder", { orderid });
    alert("Deliverd Success");
    const orders = await axios.get("/api/orders/alluserorder");
    dispatch({ type: "GET_ALL_ORDER_SUCCESS", payload: orders.data });
    window.location.href = "/admin/orderlist";
  } catch (error) {
    dispatch({ type: "GET_ALL_ORDER_FAIL", payload: error });
  }
};

export const deleteAllOrders = () => async (dispatch) => {
  dispatch({ type: "DELETE_ALL_ORDER_REQUEST" });
  try {
    await axios.delete("/api/orders");
    alert("All orders deleted successfully");
    dispatch({ type: "DELETE_ALL_ORDER_SUCCESS" });
  } catch (error) {
    dispatch({ type: "DELETE_ALL_ORDER_FAIL", payload: error });
  }
};



